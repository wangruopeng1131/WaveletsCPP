...
int downsamplingConvolution(const double *const input, const size_t N,
    ...
    if (mode == MODE_PERIODIZATION)
        return downsamplingConvolutionPeriodization(input, N, filter, F, output, step, 1);
...
static int upsamplingConvolutionValidSfPeriodization(const double *const input, const size_t N,
                                                     const double *const filter, const size_t F,
                                                     double *const output, const size_t O) {
...
int upsamplingConvolutionValidSf(const double *const input, const size_t N,
    ...
    if (mode == MODE_PERIODIZATION)
        return upsamplingConvolutionValidSfPeriodization(input, N, filter, F, output, O);
    if ((F % 2) || (N < F / 2))
        return -1;
